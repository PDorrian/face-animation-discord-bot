...
class Deep(commands.Cog):
    ...
    async def deep_create(message, cmd):
        ...
        #os.system("python " + script_dir + "/demo.py  --config " + script_dir + "/config/vox-adv-256.yaml --driving_video " + script_dir + "/driving_video/" + cmd + ".mp4 --source_image " + script_dir + "/image.png --checkpoint " + script_dir + "/checkpoints/vox-adv-cpk.pth.tar --relative --adapt_scale")


        source_image = imageio.imread(cwd + '/image.png')
        reader = imageio.get_reader('driving_video/' + cmd + '.mp4')
        ...
        generator, kp_detector = load_checkpoints(config_path='config/vox-adv-256.yaml', checkpoint_path='checkpoints/vox-adv-cpk.pth.tar')
